...
CONFIG={
        "name":"Rabenseifner", 
        "cat":"Reduce", 
        "desc":"All nodes have the data and reduced data should be in root node.",
        "ndbs":16,
        "ndbs_x":4, 
        "ndbs_y":4,
        "ncpus":8,
...
class Reduce_Raben(Scene):
    ...
    def construct_conf(self, di):
        ...
        for icpu,dbs  in enumerate(di):
            ...
            cib = ib
            for idb in dbs:
                cib[1][idb].set_fill (COLOR[icpu])
                cib[1][idb].set_opacity (0.7)
            Trf.append (Transform(ib,cib))
        self.play (*Trf)

    def construct_whitewash(self):
        """Loads initial db distro using list of lists"""
        Trf = []
        for icpu  in range(8):
            ib  = self.NODES[icpu]
    ...
    def construct (self):
        ...
        ## who is receviving.
        ## each node is only incharge of a specific chunk
        ### one
        self.construct_step (
            [0,  4,  1,  5,  2,  6,  3, 7],
            [4,  0,  5,  1,  6,  2,  7, 3],
        ...
        STEP1_TX = [1,3,5,7]
        STEP1_RX = [0,2,4,6]
        STEP1_DB = [[2, 3], [6, 7], [10, 11], [14, 15]]
        # 2
        STEP2_TX = [2,6]
        STEP2_RX = [0,4]
        STEP2_DB = [[4, 5, 6, 7], [12, 13, 14, 15]]
        # 3
        STEP3_TX = [4]
        STEP3_RX = [0]
        STEP3_DB = [[8,9,10,11,12,13,14,15]]
        ##
        self.construct_step (
