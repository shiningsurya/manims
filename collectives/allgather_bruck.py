...
# 1
STEP1_TX = [0,1,2,3,4,5,6,7]
STEP1_RX = [7,0,1,2,3,4,5,6]
STEP1_DB = deepcopy (STATE)
for i, ss in enumerate (STATE):
    irx = STEP1_RX.index (i)
    STATE[i] += STATE[irx]
# 2
STEP2_TX = [0,1,2,3,4,5,6,7]
STEP2_RX = [6,7,0,1,2,3,4,5]
STEP2_DB = deepcopy (STATE)
for i, ss in enumerate (STATE):
    irx = STEP2_RX.index (i)
    STATE[i] += STATE[irx]
# 3
STEP3_TX = [0,1,2,3,4,5,6,7]
STEP3_RX = [4,5,6,7,0,1,2,3]
STEP3_DB = deepcopy (STATE)
for i, ss in enumerate (STATE):
    irx = STEP3_RX.index (i)
    STATE[i] += STATE[irx]
